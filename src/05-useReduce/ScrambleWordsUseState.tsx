...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (guess === currentWord) {
      ...
      const nextWord = words[nextPoints];

      if (points + 1 <= GAME_WORDS.length) {
        confetti({
          particleCount: 100,
          spread: 120,
          origin: { y: 0.6 },
      ...
      //!esto esta mal
      //TODO: estoy pasando como argumentos, valores viejos, no los valores actuales que aun estan
      //TODO: en pendiente en esa ejecucion.
      //setPoints((prev) => prev + 1);
      // setCurrentWord(words[points]);
  ...
  const handlePlayAgain = () => {
    const newOrderArray = shuffleArray(GAME_WORDS);

    setWords(newOrderArray);
    setCurrentWord(newOrderArray[0]);
    setScrambledWord(scrambleWord(newOrderArray[0]));
    setGuess("");
    setPoints(0);
    setErrorCounter(0);
    setSkipCounter(0);
